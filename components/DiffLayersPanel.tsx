...
const STATUS_DOT: Record<DiffStatus, string> = {
  ...
  unchanged: 'bg-slate-300',
};

const STATUS_ROW: Record<DiffStatus, string> = {
  added: 'bg-green-50 hover:bg-green-100',
  removed: 'bg-red-50 hover:bg-red-100',
  modified: 'bg-amber-50 hover:bg-amber-100',
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-2 px-3 py-2 text-left transition-colors ${STATUS_ROW[layerDiff.status]} ${
        active ? 'ring-1 ring-inset ring-blue-400' : ''
      }`}
    >
      {/* Status dot */}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="truncate text-xs font-medium text-slate-700">{displayName}</span>
        ...
        {layerDiff.counts.total > 0 && (
          <div className="mt-0.5 flex items-center gap-2 pl-4">
            {layerDiff.counts.added > 0 && (
              <span className="text-[9px] font-medium text-green-600">+{layerDiff.counts.added}</span>
            )}
            {layerDiff.counts.removed > 0 && (
              <span className="text-[9px] font-medium text-red-600">−{layerDiff.counts.removed}</span>
            )}
            {layerDiff.counts.modified > 0 && (
              <span className="text-[9px] font-medium text-amber-600">~{layerDiff.counts.modified}</span>
            )}
          </div>
        )}
      </div>
...
export default function DiffLayersPanel({ diff, activeLayerId, onSelectLayer }: DiffLayersPanelProps) {
  ...
  return (
    <div className="flex h-full w-56 flex-shrink-0 flex-col border-r border-slate-200 bg-white">
      {/* Header */}
      ...
      <div className="flex flex-wrap gap-1.5 border-b border-slate-100 px-3 py-2">
        ...
        {counts.total === 0 && (
          <span className="text-[10px] text-slate-400">No layer differences</span>
        )}
      </div>
      ...
      <div className="border-t border-slate-100 px-3 py-2">
        <div className="mb-1 text-[9px] font-bold uppercase tracking-wider text-slate-400">Legend</div>
        <div className="flex flex-col gap-0.5">
          {(
            [
              ['added', 'Added', 'text-green-700'],
              ['removed', 'Removed', 'text-red-700'],
              ['modified', 'Modified', 'text-amber-700'],
